...
from `0.00648` to `0.01041`.  The resulting concrete candidate is
`sup H^(1/2)M_H^2 < infinity`, equivalently `M_H=O(H^(-1/4))`, uniformly in
the state and cutoff.  One-step K12 optimizations at H=8,16,32,64 have fitted
slope `-0.2754` and keep `H^(1/4)M_H` below `0.0190`; H128 gives `0.01332`.
These are finite stress values, not a theorem.  The exact local-triad rule
removes scale-separated low/high satellites, while a dense comparable-scale
band does not grow from K8 to K12.  The unresolved proof target is therefore
a coupled local high/high tail estimate, not another unstructured cutoff
scan.  An exact height-shell Gram/Schur ledger further writes `T2<=R_H D_H`.
On the K8/K12 height scans `R_H<=1.66034`, the bound is 89--98% sharp, and all
inter-shell Gram terms have the same sign.  The next analytic target is the
joint uniform bound on `H^(1/2) S_full^2 B1 R_H D_H/(Z^3P^5)`; cancellation
cannot supply it on the current extremizers.  The stronger sufficient gap
lemma `|corr(A b_i,A b_j)|<=C 2^(-|i-j|)` has been rejected as a standalone
mechanism: exact-gradient states have correlations `0.9857--0.9990` at gaps
4, 6, and 7.  Those states concentrate the common output on one or two
cutoff-wall modes, yet their full compensated PNT-12 Schur values are about
nine orders below the direct PNT stress record because the common
normalization collapses.  Ordered-triad attribution shows that the dominant
high-shell output is generated by an increasingly dense interaction set
(effective count `6.75` at K8 and `43.43` at K12), so the data do not yet give
a sparse scalable counterexample.  PNT-8 and PNT-12 remain open; PNT-13 is
rejected, and none of these finite computations is a Clay proof.
The surviving PNT-12 quantity now has its own fixed-row exact-gradient
adversary.  Sixteen K8 steps followed by zero padding and one K12 step raise
the H16 compensated Schur value from `3.71126e-4` to a new finite stress
record `4.18215e-4`.  The search differentiates the Gram row, diagonal tail,
and common normalization together; its final K12 gradient check is
`1.21e-10`.  A direct group-index VJP reduces peak K12 memory from `7.94 GiB`
to `4.88 GiB` and retains deterministic 12-worker traces.  This improves the
falsification instrument, not the logical status: no uniform PNT-12 bound is
proved.
See
