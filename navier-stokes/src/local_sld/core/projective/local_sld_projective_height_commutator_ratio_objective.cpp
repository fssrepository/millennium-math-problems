...
LocalSldProjectiveHeightCommutatorRatioObjective(
    ...
    if (threads < 1 || threads > 256) {
        throw std::invalid_argument(
            "projective height commutator-ratio threads must be 1..256");
    }
}

LocalSldProjectiveHeightCommutatorRatioObjectiveValue
LocalSldProjectiveHeightCommutatorRatioObjective::evaluate(
    const SpectralState& state) const {
    const LocalQuarticClosureObjectiveValue selected =
        LocalQuarticClosureObjective(
            dynamics_, selection_).evaluate(state);
    LocalSldProjectiveHeightCommutatorRatioObjectiveValue result;
    ...
    const ProjectiveHeightEnvelopeMoment envelope =
        ProjectiveHeightEnvelopeKernel::evaluate(
            ...
            selected.palinstrophy, false, threads_, true);
    result.height_shell_count = envelope.height_shell_count;
    result.commutator_paired_bracket_envelope =
...
SpectralIncrement
LocalSldProjectiveHeightCommutatorRatioObjective::gradient(
    const SpectralState& state) const {
    const LocalQuarticClosureObjectiveValue selected =
        LocalQuarticClosureObjective(
            dynamics_, selection_).evaluate(state);
    ...
    const ProjectiveHeightEnvelopeMoment envelope =
        ProjectiveHeightEnvelopeKernel::evaluate(
            state, selection_, selected.enstrophy,
            selected.palinstrophy, true, threads_, true);
    if (!(envelope.outer_h1_sum > 1e-30L)) {
        return result;
    }
    const SpectralReal ratio =
        envelope.absolute_component_envelope /
        envelope.outer_h1_sum;
