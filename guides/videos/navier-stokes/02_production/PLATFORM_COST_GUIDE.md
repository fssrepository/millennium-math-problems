...
The film needs six motion plates: S01–S06. S07 is a fixed hold on the supplied
S06 endpoint image and costs no video-generation credits.

The economical plan assumes:

- one output per decision cycle;
- six accepted base outputs;
...
- model: **Veo 3.1 Lite**;
- mode: **Video → Frames → Add start frame**;
- aspect ratio: **9:16**;
...
### Current Flow credit math

For non-Ultra accounts, Google currently lists:
...
| --- | ---: | ---: | ---: |
| Six Lite outputs | 60 | 2 | $0 |
| Six Lite outputs plus three Lite retries | 90 | 2 | $0 |
| Previous row plus one Fast rescue | 110 | 3 | $0 |

These estimates require one generated output per request. Flow charges per
generation, not per prompt request, and some interface modes may create two
outputs. Before clicking Generate, confirm that the displayed charge is for
one generation. If Flow forces two outputs, the six-shot Lite baseline becomes
120 credits and requires at least three free-credit days.
...
Google states that videos produced on the Free, Plus and Pro tiers contain a
visible Veo watermark. Ultra output may still contain one where local rules
require it. Flow is therefore excellent for the free scout and rough-cut stage,
but may not be the preferred final renderer when a clean public master is
required.
...
- model: **Kling 3.0**;
- mode: **Image to Video** or **Start Frame**;
- video mode: **Standard**, when the selector is shown;
- resolution: **720p**;
- duration: 5–8 seconds for this film;
- output count: **1**;
- audio: off.

Kling 3.0 supports 720p and 1080p, three-to-fifteen-second durations and image
references. The supplied prompts need only single-shot image-to-video; do not
enable multi-shot storyboarding, Soul ID or agent features.

### Expected Higgsfield cost
...
- model: **Kling 3.0**;
- mode: **Frame to Video → Start/End Frame**;
- start frame: the supplied PNG for the shot;
- output: **5 seconds or the nearest available duration, 720p, 1 output**;
- audio: off;
- multi-shot, Smart Shot, Motion Control and Director: off.

OpenArt's current Kling 3.0 surface exposes start/end-frame input and a 720p
output setting. A small signup trial may be available, but do not assume that
it will fund the complete six-shot film.
...
- Start with the free still animatic.
- Generate S02, then inspect it before generating S05.
- Never order four variants by default.
- Permit one targeted retry only after naming the visible defect.
- Do not upgrade a flaw visible only at 200% desktop zoom.
