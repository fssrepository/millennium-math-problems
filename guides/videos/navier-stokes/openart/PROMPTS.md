...
```text
Macro view inside the established cube. Orbit about thirty degrees around the
same coral filament bend with an 85mm macro lens. The large teal vortex visibly
stretches the coral core while very fine cyan ripples nearby softly lose
contrast, suggesting viscous smoothing. One continuous physical motion, no
split screen. Finish in a stable arrested composition with the bend center-right.
Keep the composition calm enough for three short method-family cards added in
post; do not depict people, portraits or written names.
```

## S04 — spectral shells

```text
The camera pulls through the fluid and reveals nested transparent Fourier-like
frequency shells surrounding the same cube, each shell a faint cyan wire volume
with tiny amber test pulses. Execute a fast smooth forward flight through the
shells, with the highest camera energy in the sequence and no random shake.
Four amber test pulses occur at visibly different shell depths. The coral
filament remains visible as the recurring anchor in the central cube. Supply
clean handles so the editor can place the audited T+ event times.
```

## S05 — deeper optimization plate

```text
Begin from the uploaded macro checkpoint reference. Hold an extremely stable
85mm macro composition of the same coral bend while the teal flow moves slowly around
it. A thin translucent amber scanning plane crosses the exact bend once,
