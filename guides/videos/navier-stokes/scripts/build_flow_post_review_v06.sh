...
output_file="${2:-${guide_dir}/renders/edit/navier-stokes-flow-post-v06-review-720p.mp4}"
...
# Keep the V04 visual overlap repair. Both the left and bottom perimeter glints
# around the 7.4 s card have been removed from the subtitle overlay.
#
# Audio changes:
# - preserve the original audio continuously through 7 s;
# - align the matching rhythmic impulses across the true 8.000 s Extend seam
#   with a 135 ms equal-power crossfade;
# - slow only the post-seam audio by 0.618%, replacing the crossfaded duration
#   and keeping the soundtrack aligned at the 29.7 s endpoint;
# - leave the already-clean 23 s join untouched.
...
[aright]atrim=start=8.000,asetpts=PTS-STARTPTS,atempo=0.993816[ar];\
[al][ar]acrossfade=d=0.135000:c1=qsin:c2=qsin,\
