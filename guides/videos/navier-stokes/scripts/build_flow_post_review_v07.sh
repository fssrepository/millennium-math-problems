...
output_file="${2:-${guide_dir}/renders/edit/navier-stokes-flow-post-v07-review-720p.mp4}"
...
# Keep the V04 visual overlap repair and the overlay cleanup.
# Preserve the original score continuously through 7 s. Repair the single
# discontinuity at 8.000 s with a two-millisecond equal-power splice: short
# enough to behave as sample repair rather than an audible musical crossfade.
# A 0.0092% tempo correction on only the post-seam portion replaces those two
# milliseconds without moving the video/audio endpoint.
...
apad=pad_dur=0.010,atrim=duration=29.700,volume=-1.2dB[aout]" \
  -map "[vout]" -map "[aout]" \
