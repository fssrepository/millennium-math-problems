...
output_file="${3:-${guide_dir}/renders/edit/navier-stokes-flow-post-v12-review-720p.mp4}"
overlay_file="${guide_dir}/04_editorial/overlays/flow-final-overlays-v02.ass"

for required_file in "${video_input}" "${audio_input}" "${overlay_file}"; do
  if [[ ! -f "${required_file}" ]]; then echo "Missing required file: ${required_file}" >&2; exit 1; fi
done
mkdir -p -- "$(dirname -- "${output_file}")"
video_tmp="$(mktemp --tmpdir="$(dirname -- "${output_file}")" navier-v12-video-XXXXXX.mp4)"
